
#include <bits/stdc++.h>

using namespace std;

/* Author: Divyanshu Kakwani */

/** Boilerplate */

// type aliases
typedef long long ll;
typedef pair<int, int> ii;
typedef vector<ii> vii;
typedef vector<int> vi;
typedef vector< vector<int> > vvi;	// for unweighted graph
typedef vector< vector< ii > > vvii;	// for weighted graph

// Constants
#define INF 1e9


// Shorthands for frequently occuring code
#define rep(i, a, b) 	for(int i = (a); i < (b); i++)
#define inpN(a, n)	rep(i, 0, n) cin >> a[i]; 
#define fill(a, v) 	(memset(a, v, sizeof a))
#define in(a, b) 	((b).find(a) != (b).end())
#define max(a, b)	((a) > (b) ? (a) : (b))
#define min(a, b)	((a) < (b) ? (a) : (b))
#define isInt(a)	int(a) == (a) ? true : false
#define el		cout << endl;
#define mp		make_pair

// Debugging macros
#define show(x)		cerr << #x << ": " << x << endl;

// Miscellaneous
#define nosync ios_base::sync_with_stdio(0);

int main() {
	int t;
	cin >> t;
	rep(i, 1, t+1) {
		int x, r, c;
		cin >> x >> r >> c;
		cout << "Case #" << i << ": ";
		if(x >= 7)
			cout << "RICHARD" << endl;
		else {
			if(x >= min(r,c)*2+1 or x >= 2*max(r,c) + 1)
				cout << "RICHARD" << endl;
			else {

				if(x==min(r,c)*2 - 1) {
					if(min(r,c) * min(r,c)	% x == 0 and (r*c - min(r,c) *min(r,c) - 2*min(r,c)) % x == 0)
						cout << "GABRIEL" << endl;
					else
						cout << "RICHARD" << endl;

				}
	
				
				else if(x== min(r,c)*2) {
					if(min(r,c) * (min(r,c)+1)% x == 0 and (r*c - min(r,c) *(min(r,c)+1) - 2*min(r,c)+1) % x == 0)
						cout << "GABRIEL" << endl;
					else
						cout << "RICHARD" << endl;

				}
				else if((r*c - x) % x == 0)
					cout << "GABRIEL" << endl;
				else
					cout << "RICHARD" << endl;
			}
		}
	}
}
